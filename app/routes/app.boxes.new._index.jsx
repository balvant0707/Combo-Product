...
export const action = async ({ request }) => {
  ...
  try {
    ...
    // After creating the box, redirect to its Specific Combo Box tab
    throw redirect(withEmbeddedAppParams(`/app/boxes/${box.id}/combo`, new URL(request.url).search));
  } catch (e) {
    if (e instanceof Response) throw e; // re-throw redirects
    console.error("[app.boxes.new._index] createBox error:", e);
...
/* ─────────────────────────── Component ─────────────────────────── */
export default function CreateBoxSettingsPage() {
  ...
  function openPicker() { setProductSearch(""); searchFetcher.load(withEmbeddedAppParams("/app/boxes/new", location.search)); setShowPicker(true); }
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "16px" }}>
        <button
          type="button"
          disabled={isSaving}
          onClick={() => { const f = document.getElementById("create-box-form"); if (f) f.requestSubmit(); }}
          style={{ background: "#2A7A4F", color: "#fff", border: "none", borderRadius: "6px", padding: "10px 24px", fontSize: "14px", fontWeight: "700", cursor: isSaving ? "not-allowed" : "pointer", opacity: isSaving ? 0.7 : 1 }}
        >
          {isSaving ? "Saving..." : "Save & Continue to Combo"}
        </button>
      </div>
      ...
      {errors._global && (
        <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "5px", padding: "12px 16px", marginBottom: "16px", color: "#991b1b", fontSize: "13px", display: "flex", alignItems: "center", gap: "8px" }}>
          <span style={{ fontSize: "16px" }}>⚠</span>{errors._global}
        </div>
      )}

      <Form id="create-box-form" method="POST" action={`/app/boxes/new${location.search}`} encType="multipart/form-data">
        ...
        <s-section>
          ...
          <div style={{ marginBottom: "28px" }}>
            <div style={sectionHeadingStyle}><span style={{ fontSize: "15px" }}>📋</span> Basic Information</div>
